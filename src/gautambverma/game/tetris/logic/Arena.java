...
import gautambverma.game.tetris.logic.Shape;
...
public class Arena {
		...
		while(accumulator > UPDATE_INTERVAL) {
			if(isEmpty()) shape.update();
				...
				while( callAgain ) {
					...
					if(!callAgain) {
						moveDownHangingBlocks();          /* Blocks with no support to move down*/ 
						callAgain = clearCompletedLine(); /* Check again for the Completed Line */
					}
				}
	...
	/**
	 * Move down all blocks which have no blocks in beside or up and down.
	 * */
	protected void moveDownHangingBlocks() {
		
		for(int i = 1; i<(AppConst.ARENA_GRID_HEIGHT-1); i++) {	
			for(int j = 1; j<(AppConst.ARENA_GRID_WIDTH-1); j++) {
				if(gameArea[i][j] && !gameArea[i-1][j]
								  && !gameArea[i][j-1]
								  && !gameArea[i+1][j]	
								  && !gameArea[i][j+1]) {
					moveBlockDown(i+1, j);
			...
			// Oth Column
			if(gameArea[i][0] && !gameArea[i-1][0]
					  && !gameArea[i][1]
					  && !gameArea[i+1][0]) {
				moveBlockDown(i+1, 0);
			}
			// 13th Column i.e. last column
			if(gameArea[i][0] && !gameArea[i-1][AppConst.ARENA_GRID_WIDTH-1]
					  && !gameArea[i][AppConst.ARENA_GRID_WIDTH-2]
					  && !gameArea[i+1][AppConst.ARENA_GRID_WIDTH-1]) {
				moveBlockDown(i+1, AppConst.ARENA_GRID_WIDTH-1);
			}
		}
	}

	/*
	 * Move block in Jth Column till their is empty space
	 * */
	protected void moveBlockDown(int k, int j) {
		while(k<AppConst.ARENA_GRID_HEIGHT && !gameArea[k][j]) {
			gameArea[k][j] = gameArea[k-1][j];
			gameArea[k-1][j] = false;
			k++;
		}
	}
	/*
	...
	boolean clearCompletedLine() {
		...
		if(i!=-1) {
			// Erase the Completed line and bring every other drawn block one unit down. (i.e increase y value)
			if(Settings.soundEnabled)
