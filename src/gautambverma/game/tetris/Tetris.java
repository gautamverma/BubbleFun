 ...
 * */
package gautambverma.game.tetris;
...
import gautambverma.com.game.impl.AndroidGame;
import gautambverma.game.tetris.screen.LoadingScreen;
...
public class Tetris extends AndroidGame {
	...
	SKBaseListener achievementListener = new SKBaseListener() {
		...
		public void onResponse(SKBaseResponse arg0) {
			...
			if (arg0.getStatusCode() == 0) {
				...
				showAchievementOpenedToast();
			}
			else {
				achievementOpened = false;
				showAchievementOpenedFailedToast();
			}
			notifyThreadsWaitingOnSimpleLock();
		}
	};
	...
	protected boolean haveInternet() {
		...
		if (info.isConnected())
			return isConnectionActive(URI.create("http://www.google.com"));
		return false;
	}

	/***
	 * We make a Http Request and if response comes it means connection is can be used to data transfer 
	 * */
	protected boolean isConnectionActive(URI url) {
	...
	protected void showAchievementOpenedFailedToast() {
		Toast noConnectionToast = Toast.makeText(tetris,
				R.string.achievement_failed, Toast.LENGTH_SHORT);
		noConnectionToast.show();
	}
	
	protected void showAchievementOpenedToast() {
		Toast noConnectionToast = Toast.makeText(tetris,
				R.string.achievement_opened, Toast.LENGTH_SHORT);
		noConnectionToast.show();
	}
	
	protected void showBusy() {
		busyDialog = new ProgressDialog(tetris, ProgressDialog.STYLE_SPINNER);
		busyDialog.setIndeterminate(true);
		busyDialog.setMessage(getString(R.string.loading));
		busyDialog.show();
	}
